...
class PipelineConfig:
    ...
    #   labels      — allowed values for output.label, injected into the VLM
    #                 prompt so the answer's verdict suits the task (optional;
    #                 defaults to the match/mismatch set when omitted)
        ...
        default_factory=lambda: [
            {
                ...
                "labels": ["clear", "partial", "unclear"],
            },
            {
                "task_type": "element_detection",
                "images": ["site"],
                "instruction": (
                    "현장 사진에서 보이는 구조 요소(보, 기둥, 슬래브, 벽, 계단 등)를 "
                    "찾아 종류별로 나열하고 각 식별 근거를 제시하라."
                ),
                "labels": ["detected", "partial", "none"],
            },
            {
                "task_type": "material_identification",
                "images": ["site"],
                "instruction": (
                    "현장 사진에 나타난 주요 구조 재료와 공법을 식별하라 "
                    "(예: 철근콘크리트, 철골, 복합구조)."
                ),
            ...
            },
            {
                ...
                "images": ["site"],
                "instruction": (
                    "이 현장의 시공 진행 단계(기초/골조/마감 등)와 대략적인 완료도를 "
                    "근거와 함께 평가하라."
                ),
                "labels": ["foundation", "structure", "finishing", "completed"],
            },
            {
    ...
    def file_output_dir(self, stem: str, kind: str = "",
        ...
        *stem* is the input file name without extension. Every dataset derived
        ...
        ``dapt_training_data.jsonl``, ``vlm_training_data.jsonl``), so a
        ``_sft`` / ``_dapt`` / ``_vlm`` suffix never prevented a collision — it
        only doubled the directory count. *kind* is kept for call-site

"""Command-line entry point for the AEC synthetic-dataset pipeline.

Installed as the ``aec-pipeline`` console script (see pyproject.toml). Also
runnable in-repo via ``python main.py`` (a thin shim that calls :func:`main`).
"""
...
def _build_config(args: argparse.Namespace, base):
    ...
    from .config import PipelineConfig  # local import to keep startup fast
...
def main() -> int:
    ...
    # ── Full pipeline run ────────────────────────────────────────────────
    from .pipeline import AECPipeline  # deferred import
